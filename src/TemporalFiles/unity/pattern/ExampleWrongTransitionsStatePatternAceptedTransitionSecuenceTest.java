...
public class ExampleWrongTransitionsStatePatternAceptedTransitionSecuenceTest {
	...
	private State executeActionSecuence (List<Integer> transitonOrder) throws Exception {
		...
		if (initialStateClassName.compareTo("tfg.sw.implementationExample.pattern.state.ExampleWrongTransitions.State1") != 0) 
			return null;
	...
	private List<String> getFinalStatesCanonicalNames () {
		...
		result.add("tfg.sw.implementationExample.pattern.state.ExampleWrongTransitions.State1");
		result.add("tfg.sw.implementationExample.pattern.state.ExampleWrongTransitions.State3");
